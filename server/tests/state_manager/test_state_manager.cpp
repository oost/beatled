...
TEST_CASE("StateManager tempo", "[state_manager]") {
  ...
  SECTION("Clients can be registered") {
    ...
    ClientStatus::Ptr cs = std::make_unique<ClientStatus>();
    ClientStatus *cs_raw_ptr = sm.register_client(client_address, board_id);
    REQUIRE(cs_raw_ptr);
    REQUIRE(sm.client_status(client_address) == cs_raw_ptr);
    REQUIRE(cs_raw_ptr->board_id == board_id);
    REQUIRE_FALSE(sm.client_status(client_address2));
  }
}