...
#include <time.h>
// #include "config.h"
#define AUBIO_UNSTABLE_API 1
#define AUBIO_TESTS_SOURCE
...
#define PRINT_ERR(format, args...)                                             \
  fprintf(stderr, "AUBIO-TESTS ERROR: " format, ##args)
#define PRINT_MSG(format, args...) fprintf(stdout, format, ##args)
#define PRINT_DBG(format, args...) fprintf(stderr, format, ##args)
#define PRINT_WRN(format, args...)                                             \
  fprintf(stderr, "AUBIO-TESTS WARNING: " format, ##args)
#endif
...
int check_source(char *source_path) { return access(source_path, R_OK); }

int create_temp_sink(char *sink_path) { return mkstemp(sink_path); }

int close_temp_sink(char *sink_path, int sink_fildes) {
  int err;
  if ((err = close(sink_fildes)) != 0)
    return err;
  if ((err = unlink(sink_path)) != 0)
    return err;
  return err;
}
...
int check_source(char *source_path) { return _access(source_path, 04); }
