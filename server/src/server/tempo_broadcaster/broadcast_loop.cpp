...
BroadcastLoop::BroadcastLoop(std::shared_ptr<asio::ip::udp::socket> socket,
    ...
    : alarm_period_{alarm_period}, count_{0},
      timer_{asio::high_resolution_timer(socket->get_executor(), alarm_period)}, socket_{socket},
      prepare_buffer_{prepare_buffer},
      broadcast_address_{
...
void BroadcastLoop::do_broadcast() {
  ...
  socket_->async_send_to(asio::buffer(response_buffer_ptr->data(), response_buffer_ptr->size()),
                         broadcast_address_,
