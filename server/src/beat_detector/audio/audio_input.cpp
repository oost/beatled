...
AudioInput::AudioInput(AudioBufferPool *audio_buffer_pool, double desired_sample_rate,
                       unsigned long frames_per_buffer)
    : AudioInterface(audio_buffer_pool, desired_sample_rate, frames_per_buffer) {}
...
const PaStreamParameters *AudioInput::get_input_parameters() {
  ...
  for (int api_idx = 0; api_idx < num_apis; api_idx++) {
    ...
    SPDLOG_INFO(" - API {}: type: '{}',  name: '{}', devices: '{}'", api_idx, (int)api_info->type,
  ...
  input_parameters_.device = Pa_GetDefaultInputDevice(); /* default input device */
  ...
  const PaDeviceInfo *default_input_device_info = Pa_GetDeviceInfo(input_parameters_.device);

  SPDLOG_INFO("Starting listening on device: {} (API: {}, index: {})",
              default_input_device_info->name, default_input_device_info->hostApi,
  ...
  input_parameters_.suggestedLatency = default_input_device_info->defaultLowInputLatency;
