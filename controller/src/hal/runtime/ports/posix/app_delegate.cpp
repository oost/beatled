...
void MyAppDelegate::applicationDidFinishLaunching(NS::Notification *pNotification) {
  ...
  // Click-drag in the content area orbits the hat; title-bar clicks still
  // move the window (the monitor filters on the content height).
  mouse_monitor::install(_pViewDelegate->renderer(), frame.size.height);

  _pWindow->setContentView(_pMtkView);
  _pWindow->setTitle(
      NS::String::string("Pico Beatled Simulator", NS::StringEncoding::UTF8StringEncoding));
