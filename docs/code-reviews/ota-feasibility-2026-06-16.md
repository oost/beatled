...
## Target A — `esp32-freertos` (ESP32-S3): well-supported
...
- **Risks:** flash size for dual app slots (fine on typical 4–8 MB ESP32-S3
  modules); getting rollback/validation right so a bad image self-recovers.
...
- **ESP32-S3 (esp32-freertos):** yes — native ESP-IDF OTA; the pragmatic target
  if OTA matters. Gated on the ESP32 port landing.
