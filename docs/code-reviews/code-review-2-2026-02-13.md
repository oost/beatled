...
## Resolution status (marked 2026-06-11)

Every open finding below is resolved except a small accepted-deferral set.
Markers: ✅ fixed (verified in-tree or in the 2026-06 review-fixes batch),
⏸ deliberately deferred (accepted, not planned), ⚠ open/external. The
consolidated cross-review tracker is
`docs/code-reviews/code-review-unfixed.md`.

- ⏸ Deferred: M14 (centralized field validation), M15 (LED/registry
  decoupling), M16 (per-port HAL queue allocation — correct per RTOS),
  M19 (`API_HOST` module state — bounded, validated on write), M40
  (vcpkg version overrides), L6 (route lazy-loading), L9 (bundle-size
  analysis), L11 (npm caret ranges), L12 (Ruby bundler version).
- ⚠ Open/external: H10 (required-status-check gating is a GitHub
  branch-protection setting, not expressible in-tree; all CI workflows
  do run their tests).
